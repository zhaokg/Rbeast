...
def plot_o(h, ytitle, has, clr, x, t, t2t, Y, CI ):
    ...
    #
    # https://github.com/shap/shap/issues/3368
    #  - use_line_collection=True argument is deprecated since matplotlib 3.6.0.
    #  - The last version where this argument exists is 3.7.3
    ...
    # A "Pythonic" approach is to try the modern syntax first and catch the TypeError if it fails on an older version     
    try:
        # Modern approach (3.3+)
        h.stem( t, Y,  linefmt='-', markerfmt=None)
    except TypeError:
        # Fallback for very old versions (pre-3.3)
        h.stem( t, Y,  linefmt='-', markerfmt=None ,use_line_collection=True) 
...
from numpy            import ndarray, min, max,ndarray,flip, isnan, round, sum, arange
from numpy            import concatenate as c, array as np_array, where as np_where, sqrt as np_sqrt, ndim as np_ndim
from   .extractbeast  import extract     as extractbeast

# Convert a 1-element ndarray to a scalar
#  - float(np.array([1])):  Conversion of an array with ndim > 0 to a 
...
def get_tcp(x, ncpStat):
    ...
    ncp  = round(ncp)
    ncp  = int( simplify(ncp) ) # Simplify converts a ndarray to a scalar.
    ncpPr, cpPr,cpChange, Prob = (cmpnt.ncpPr, cmpnt.cpPr ,cmpnt.cpAbruptChange, cmpnt.cpOccPr)
    Prob1 = c( [Prob,Prob - Prob])
